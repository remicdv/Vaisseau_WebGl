...
var ct = 0;  // number of textures loaded
...
function corpsRender(){
	...
	setMaterialDiffuseValeur(0.9,0.9,0.9);
	setTextureToRender(texID1);
    corpsDroit.render();
...
function setTextureToRender(texture)
{
	setMaterialDiffuseValeur(0.6, 0.6 ,0.6);
	gl.uniform1f(gl.getUniformLocation(prog, "isTexture"), 1.0);
...
// Méthode qui permet de ne plus render de texture, en passant 0 a haveTexture dans le Fragment Shader
function disableTextureRender()
{
	gl.uniform1f(gl.getUniformLocation(prog, "isTexture"), 0.0);
}


// Méthode qui permet de donner les trois couleurs au diffuse du material
function setMaterialDiffuseValeur(a, b, c)
{
	gl.uniform4fv(gl.getUniformLocation(prog, "ambientProduct"), flatten(mult(lightAmbient, vec4(0, 0,0,1.0))));
    gl.uniform4fv(gl.getUniformLocation(prog, "diffuseProduct"), flatten(mult(lightDiffuse, vec4(a, b, c, 1.0))));
